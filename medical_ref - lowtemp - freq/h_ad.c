        ...
        if (guc_ad_fault_cnt[guc_ad_index] >= 100)
        {
            ...
            switch (guc_ad_index)
            {
                case 0:  //TE
                    ...
                    break;
                case 1:   //TD
                    if (bflg_TD_fault == 0)   //下传感器故障
                    {
                        bflg_TD_fault = 1;
                        gus_trip_code1 |= TD_S_CODE;
                    }
                    break;
                case 2:    //TU
                    if (bflg_TU_fault == 0)  //上传感器故障
                    {
                        bflg_TU_fault = 1;
                        gus_trip_code1 |= TU_S_CODE;
                    }
                    break;
                case 3:  //TC
                    if (bflg_TC_fault == 0)   //冷凝传感器故障
                    {
                        bflg_TC_fault = 1;
...
{
    int32_t lsl_tmp;
    int16_t lss_TA = gss_TA,lss_TD = gss_TD,lss_TU = gss_TU;
    //----------------------------------
    //gss_THW = get_temp1(gus_ad_aver_buffer[0]) + gss_factory_CA;     //添加环温校准
             
    gss_TD = get_temp5(gus_ad_aver_buffer[1]) + gss_factory_parm_value[DISP_FAC_Ct2]; 
    gss_TD = (((lss_TD*3) + gss_TD)>> 2);        
    gss_TU = get_temp5(gus_ad_aver_buffer[2]) + gss_factory_parm_value[DISP_FAC_Ct1]; 
    gss_TU = (((lss_TU*3) + gss_TU)>> 2);        
    
    gss_TC = get_temp1(gus_ad_aver_buffer[3]) + gss_factory_parm_value[DISP_FAC_Ct5];    
    
    gss_TE = get_temp2(gus_ad_aver_buffer[0]) + gss_factory_parm_value[DISP_FAC_Ct4];   
    
    gss_TA = get_temp3(gus_ad_aver_buffer[8]) + gss_factory_parm_value[DISP_FAC_Ct3];   //添加校准  在temp3中放大的
    ...
    gss_VAC = ((int16_t) (lsl_tmp)) + gss_factory_parm_value[DISP_FAC_CU];                   //添加电压校准
...
int16_t get_temp1(uint16_t lus_ad)      //根据ad值查表得到温度值1
{
    ...
    return (temp1_for_ad[lss_temp_index]*10);
}
//------------------------------------------------------------------------------
/*******************************************************************************
函数功能: 
...
int16_t get_temp2(uint16_t lus_ad)      //根据ad值查表得到温度值2
{
    ...
    return (temp2_for_ad[lss_temp_index]*10);
}
//------------------------------------------------------------------------------
/*******************************************************************************
函数功能: 
