...
{   
    ...
    //测试模式
    if (bflg_test_mode == 1)       //如果是测试模式--要在通讯之前
    {
        test_mode_com_deal();      
    ...
    //----------------------------------
    if (bflg_com_tx_ok == 1)  //bflg_com_allow_rx 之前发完主循环大于5ms
    {
        bflg_com_tx_ok = 0;
    ...
    {
        bflg_bus_rx_busying = 1;  //接收时总线占用计时
    ...
    //{
    else if (bflg_bat_charge_update == 1)           //每10s更新一次,充电情况
    {
...
void timer_op(void)      //定时程序，在主循环程序中调用
{
    ...
    if (guc_1s_timer == 0)
    {
        ...
        //血液相关
        if(bflg_compressor_first_power_delaytime == 0) //上电延时结束
        {
...
/*void input_pin_delaytime(void)     //输入引脚延时程序，在10ms定时程序中调用
{
    ...
    if (BAT_FED_IN_PIN == 1)  //peak 电池未反接
    {
        guc_bat_fed_pin_high_delaytimer = 0;
        
        if (bflg_alarm_bat_fed == 1)       // peak 反接报警标志
        {
            ...
            if (guc_bat_fed_pin_low_delaytimer >= 100) //1s
            {
                ...
                bflg_alarm_bat_fed = 0;     //peak 清反接报警标志
